 ...
 * 2023-01-10
 */
function 构建改良型浏览器存储器(作为内核的标准存储器系本地存储器) {
    ...
    const 改良型存储器 = {
        ...
        保存条目(条目名, 原始值, 形制要求) {
            ...
            const 保质期_以秒计_给出的值 = 形制要求.保质期_以秒计
            let 保质期_以秒计
            if (typeof 保质期_以秒计_给出的值 !== 'number') {
                保质期_以秒计 = NaN
            }
            else {
                保质期_以秒计 = 保质期_以秒计_给出的值
            }
            // 此处做判断是为了避免 保质期_以秒计 取值为 NaN。不能采用 NaN 是因为 JSON.stringify 会将 NaN 替换成 null 。
            if (!(保质期_以秒计 >= 0.001)) {
                保质期_以秒计 = null
            }
            const 单项 = {
        ...
        当即取此条目(条目名, 形制要求) {
            ...
            if (标志性属性名列表.some(属性名 => !(属性名 in 处理得到的对象))) {
            ...
            const { 原始值, 时间戳, 保质期_以秒计 } = 处理得到的对象
            let 已过保质期 = false
            if (保质期_以秒计 !== null) {
                const 当下 = Date.now()
                已过保质期 = 当下 >= 时间戳 + 1 * 保质期_以秒计
            }
            if (已过保质期) {
                内核.removeItem(条目名)
                return 取值失败时专用的返回值
        ...
        取此条目(条目名, 形制要求) {
            const 原始结果 = 改良型存储器.当即取此条目(条目名, 形制要求)
            if (原始结果.已成功) {
